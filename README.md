...
| 11 | `11_combine_data.py` | Merges sources and applies `record_overrides.csv` for final data consolidation. | `Processed/processed_wowhead_data.csv`, `Extracted/petopia_data.csv`, `Processed/final_notes.csv`, `Manual/skip_display_ids.csv`, `Manual/record_overrides.csv` | `Processed/pet_data.csv` |
...
- **Location Updates**: Step 10 applies manual location corrections from `location_updates.csv` to fill in missing zone/layer/uiMap data, correct incorrect uiMapIds, and split single source rows into multiple zone/layer entries when an NPC appears in multiple areas.
- **Coordinate Thinning**: Step 10 also thins coordinates by removing redundant points that are closer than 2.0 units to each other, prioritizing points with the highest neighbor density, to reduce data size while preserving coverage.
