...
1. **Extract** — scrape raw data from Petopia/Wowhead into CSVs:
   `_extract_petopia_npcs.py`, `_extract_petopia_data.py`,
   `_extract_wowhead_families.py`, `_extract_wowhead_npcs.py`,
   `_extract_wowhead_data.py`.
2. **Clean** (`_clean_petopia_data.py`, `_clean_wowhead_data.py`,
   `_combine_data.py`) — apply hand-curated `Manual/*.csv` correction
   files (skip lists, ID remaps, keyword/location/spell overrides) on
   top of the raw scrape. `_extract_wowhead_npcs.py` →
   `_update_npcs.py` → `_extract_wowhead_data.py` is one continuous
   chain: `_update_npcs.py` applies `npcs_updates.csv` to
   `_extract_wowhead_npcs.py`'s raw scrape, `_extract_wowhead_data.py`
   reads that corrected list to pull coordinates and display IDs.
3. **Combine**: `_combine_data.py` — merges into one master `pet_data.csv`.
4. **Generate** — serialize the master data into
   `Output/{Abilities,Models,Coords,Conditions,Notes}Data.lua`:
   `_generate_abilities_lua.py`, `_generate_models_lua.py`,
   `_generate_coords_lua.py`, `_generate_conditions_lua.py`,
   `_generate_notes_lua.py`.
...
`_extract_wowhead_families.py`, not a `Manual/` CSV.
...
`1x_generate_*.py` scripts no longer sync as a side effect of
...
- **`_extract_wowhead_data.py`** is the reference implementation: a
...
- **`_extract_petopia_data.py`** ports the same lock/backoff/stop_event/
...
- **`_extract_wowhead_npcs.py`** has a lighter single-threaded
  exponential backoff on 403s only (no cross-thread coordination).
- **`_extract_petopia_npcs.py`**, **`_extract_wowhead_families.py`**,
  **`_generate_abilities_lua.py`** don't retry at all — smaller, less
  contentious endpoints, re-run manually if they fail.

If another scraper needs the same treatment, copy the pattern from
`_extract_wowhead_data.py` (or `_extract_petopia_data.py`, which is the
more minimal port) rather than inventing a new one.

`--delay` is accepted by both `_extract_wowhead_npcs.py` and
`_extract_wowhead_data.py`; it overrides the built-in
...
it silently collapses `_extract_wowhead_data.py`'s randomized jitter
range to a fixed value on every run, defeating the anti-detection jitter.
